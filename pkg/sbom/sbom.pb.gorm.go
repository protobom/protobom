...
type ExternalReferenceORM struct {
	Authority string `gorm:"type:text;primaryKey"`
	Comment   string `gorm:"type:text;primaryKey"`
	NodeId    *string
	Type      int32
	Url       string `gorm:"type:text;primaryKey"`
}
...
type ToolORM struct {
	MetadataId *string
	Name       string `gorm:"type:text;primaryKey"`
	Vendor     string `gorm:"type:text;primaryKey"`
...
func DefaultListPerson(ctx context.Context, db *gorm.DB) ([]*Person, error) {
	...
	db = db.Where(&ormObj)
	db = db.Order("email, is_org, name, phone, url")
	ormResponse := []PersonORM{}
	if err := db.Find(&ormResponse).Error; err != nil {
		return nil, err
	}
